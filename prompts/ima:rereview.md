...
`$@` must supply the approved plan, implementation, test, original review, resolution evidence, and lifecycle identity. Use a fresh, product-read-only reviewer path. Verify only whether the confirmed findings were resolved; do not edit code, assign new remediation, reopen unrelated scope, or treat unsupported concerns as findings. Report missing evidence as blocked rather than guessing.
