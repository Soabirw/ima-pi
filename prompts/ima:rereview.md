...
`$@` must supply the approved plan, implementation, test, original review, resolution evidence, and lifecycle identity. Test-origin repairs without an original formal review do not qualify for rereview; after their passing retest, they require `/ima:review` as a fresh initial review. Load `code-review` for the same evidence and verification standards, then use the eligible existing reviewer continuation recorded by the original review; do not delegate a fresh reviewer. In a cycle phase host, call `ima_agent_follow_up` with that opaque `Reviewer continuation reference` and a bounded literal rereview brief. If the reference is absent, belongs to a different lifecycle, has contract drift, is unavailable, or cannot resume, persist `BLOCKED` rather than replacing it. Verify each confirmed finding only against its required outcome and the bounded integration surface changed by its resolution. Preserve each original `REVIEW-NNN` ID; when retaining or sharpening a finding, append an implementation-grade corrective handoff under that ID rather than merely restating the failure without corrective instructions. The handoff must identify exact files/symbols, control/data/error behavior, tests and acceptance checks, constraints, rejected alternatives, and resolution dependencies. Assign the next unused ID only to an independently verified regression caused by the resolution, with the same complete corrective handoff. Do not edit code, reopen unrelated scope, or redesign; the prohibition on unrelated redesign must never suppress the corrective instructions a retained finding needs. Apply the `code-review` request-changes gate, and do not treat unsupported concerns as findings. Report missing evidence as blocked rather than guessing.
