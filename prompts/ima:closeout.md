...
Preserve the normalized canonical colon form in every result and handoff. In every fresh closeout session, call `ima_lifecycle_recall` separately for each required phase using the exact lifecycle key, exact `phase`, and `limit: 20`: always `plan`, `implementation`, `test`, `decision`, and `document`; the final approval path's `review` or `rereview`; and `resolution` when that review path requires it. The `phase: decision` recall is separate evidence. When idempotency or prior terminal evidence requires it, make one optional, separate `phase: closeout` recall for prior closeout evidence; it is not document evidence and must never be relabeled as `document`. Every listed phase call, including `decision` and optional prior `closeout`, uses its exact selector and the same `limit: 20`, exact-20 saturation block, package-owned selector `ima_lifecycle_get`, complete-evidence validation, and existing P/R/S and lineage rules. Do not make one aggregate lifecycle-wide recall and do not combine phase selectors. Each result is descriptors only: for every selected descriptor from that phase, call `ima_lifecycle_get` with only its exact `lifecycleKey`, `phase`, and `artifactId`. Never reconstruct or send its `recordKey`, `contentHash`, `summary`, or `reference` as get arguments; the package freshly recalls the exact phase, requires one unambiguous non-saturated match, and projects the closed proof internally before exact get. If **any one phase-specific** recall returns exactly 20 descriptors, treat that phase as potentially overflowed and `BLOCKED`; do not truncate, aggregate, or use another phase's smaller result to clear it. More than 20 descriptors across distinct phase calls is acceptable only when every individual phase call returned fewer than 20. Accept a record only after its complete result verifies lifecycle/source identity, requested phase and terminal outcome, returned `artifactId`/`recordKey`, content hash, and read reference. The public pair derives checkout-pin authority or, only while genuinely unpinned, exact all-phase historical Tier-1 Qdrant authority. Reuse a verified lifecycle key; for Plane only, when no artifact establishes one, use the documented `ima-pi:plane:<workspace>:<PROJECT>-<seq>` convention. A `lifecycle:` source requires this exact recall-then-get route. A `vestige:` source may read only the cited memory to recover an explicit lifecycle identity, then must use the same public pair; Vestige is never a lifecycle fallback. Pending, inaccessible, unavailable, ambiguous, incomplete, corrupt, stale, mismatched, overflowed, cancelled, or unverifiable hydration or public-read evidence is `BLOCKED`. Never use exact Tier-1 Qdrant manifest recall or selected direct detail retrieval directly. Do not retry, fall back, migrate, switch providers, mix evidence, use `ima_corpus_*`, call a provider-native read, use a generated SDK namespace, or discover storage through MCP.

Before preparing the final action overview, freshly resolve exact P/R/S through that descriptor-recall-then-exact-get sequence: `P` is the verified provider-pin anchor, `R` is the original lifecycle-seed root derived only from P's complete verified record, and `S` is P's stable source identity. P never replaces R; only `plan` and `decision` may be rootless lifecycle seeds. A stale, invalid, or rootless phase other than `plan` or `decision` P is `BLOCKED` before the overview or any tracker effect. A decision seed never replaces the distinct approved technical `plan` required for closeout. Do not infer, repair pins, migrate, repin, select a provider, fall back, or perform historical rewriting.

Then directly verify through that sequence the approved plan, implementation, test, final review or rereview, and canonical `document` artifacts for the same lifecycle identity. Retrieve every candidate needed to establish each phase's unique coherent lineage and select the latest implementation, passing test, and final approved review/rereview; every continuation, including a later plan or approval receipt, must carry exact R/S. An approval receipt preserves the original R/S and never becomes a replacement root. If those cannot be uniquely proven from complete verified artifacts and their prior references, block. The document artifact must establish completed documentation evidence and no unresolved Critical or Warning security finding. A `closeout` artifact is never relabeled as documentation here; narrowly verified historical compatibility is limited to existing cycle read/reconciliation evidence handling. Reuse every verified prior artifact ID and logical record key, including the plan and any decision artifacts; reference them without embedding their full artifacts. Fail closed if the required evidence cannot be established.
...
After approval, execute the approved actions without asking for individual confirmations. Before each effect, verify that its target and expected change still exactly match the approved overview. Before each tracker effect, freshly revalidate exact P/R/S. Stop on contradiction, failed verification, stale evidence, or any required change to an approved action; do not continue with later actions. If a change is needed, present a revised complete overview and obtain one new aggregate approval. Never expand the approved target or infer authority for an unlisted action.
...
Treat closeout as complete only after `ima_lifecycle` verifies provider-native persistence and direct read-back; this verification is required regardless of provider. Only when Qdrant is the pinned provider must it additionally verify deterministic manifest/chunk detail reassembly. Final response must distinguish completed actions from recommendations and unperformed human-owned work, state the canonical source, lifecycle key, artifact ID, record key, receipts, blockers, and residual risk, then stop.
