...
Before preparing the final action overview, freshly resolve exact P/R/S through that descriptor-recall-then-exact-get sequence: `P` is the verified provider-pin anchor, `R` is the original plan root derived only from P's complete verified record, and `S` is P's stable source identity. P never replaces R; only the original `plan` may be rootless. A stale, invalid, or rootless non-plan P is `BLOCKED` before the overview or any tracker effect; do not infer, repair pins, migrate, repin, select a provider, fall back, or perform historical rewriting.
