...
Infer one primary intent: brainstorm or research, plan or implement, review or audit, or a quick request. For an ambiguous Jira-only source, ask one focused clarification. Use `ima_context` or targeted Jira or memory reads only when a supplied reference requires it. Preserve source facts and mark unresolved placeholders or questions rather than inventing requirements.
