...
- **60 packaged skills and 38 `/ima:*` prompt templates** for engineering, web, WordPress, IMA, MCP, research, and operational workflows.
...
- **Memory-aware project work:** Pi's active global `AGENTS.md` carries current user preferences; Serena provides stable project context through same-connection, fail-closed MCP bootstrap; Vestige retains only cited legacy evidence and T7 migration sources; and BookStack, Qdrant, Serena, and Markdown are the lifecycle authorities. None are bundled services.
...
`ima_lifecycle_recall({ lifecycleKey, phase?, limit? })` accepts `limit` 1–50 and defaults to 50. A successful public result contains at most 49 verified descriptors: 49 terminal records can succeed, while exactly 50 terminal records are internally complete but public-saturated and return no descriptors. Each descriptor contains `lifecycleKey`, exact `phase`, `artifactId`, `recordKey`, `contentHash`, a bounded `summary` (at most 2,000 UTF-8 bytes), and a closed `reference` proof. The recommended model-facing call is `ima_lifecycle_get({ lifecycleKey, phase, artifactId })`: the package freshly recalls that exact phase, requires one unambiguous non-saturated match, projects the descriptor proof internally, and returns one complete verified artifact. The complete descriptor form remains accepted for compatibility and is verified strictly, but models must not reconstruct or resend its proof fields. The public `document` phase is exact and never expands to `closeout`.
...
For Qdrant lifecycle recall, a valid non-null `next_page_offset` blocks before direct detail reads and is exposed only through the fixed safe provider diagnostic `provider_verification_failed` with `step: recall` and `reason: recall_overflow`. Malformed continuation metadata remains the provider-boundary `response_invalid` classification, and missing or corrupt detail chunks remain `record_incomplete`; neither classification exposes raw provider data or partial descriptors.
...
Provider preferences and lifecycle identifiers are **non-secret variables**; credentials and tokens are **secrets**. Pins, Markdown evidence, local Qdrant state, machine Serena state (including private session state and checkout-local Serena configuration), and retained read output are **local-only values**. Configured provider service, project, or workspace destinations—including Serena MCP—are **platform bindings** where applicable and are never public read inputs.
...
For exactly one trusted-project assignment, a parent may attach optional `verifications`: each parent-minted, assignment-scoped entry selects an `npm` or Composer manifest script, repository-contained `cwd`, preapproved arguments, and a parent-selected `1,000`–`360,000` ms timeout. It is admitted only to an agent with `write` or `test-write` authority that declares `test`; the child can call only `test({ id })`. Admission validates the closed schema, role/tool/project trust, IDs, paths, symlinks, manifest, and script, then snapshots and revalidates the manifest/script before fixed, no-shell argv execution. Install/update, arbitrary executable/command, environment, URL, and credential fields are not expressible. Without an admitted entry, no typed `test` tool is exposed.

Verification shares the assignment mutation queue. Cancellation or timeout immediately latches coordinator-visible unsafe state, blocks later assignment effects and retry/reuse, waits at most `1,000` ms to observe executor settlement, and reports cleanup-unverified rather than falsely declaring cleanup settled; late rejection is observed. Output is sanitized before 200-line/16 KiB bounded truncation in both content and details result channels, including supported structured sensitive fields and complete supported authorization-header values. Focused continuations receive no typed test authority. This is not an OS sandbox: trusted manifest scripts retain host filesystem and network authority; no descendant-termination or universal-secret-detection guarantee is made.

The timeout and script metadata are **non-secret variables** (the timeout is a parent-selected runtime capability value); provider credentials are **secrets**; session and provider-pin state are **local-only values**. No **platform binding** is introduced.
...
`ima_context` uses package-owned direct MCP sessions for Serena and cited Vestige preference-source boundaries. Serena capability discovery and accepted calls share one connection; when the advertised tools require a private session, instructions load before activation/list/read/write and every present session carrier must agree. Legacy no-discovery callers retain the unparameterized activation-first sequence. Both modes require exact existing-checkout activation proof, never expose private session state, and load the six standard memories (`core`, `conventions`, `tech_stack`, `suggested_commands`, `task_completion`, and `memory_maintenance`) when listed. Schema/session failure blocks without reconnect, fallback, or effectful retry. Native `ima_corpus_*` tools own bounded institutional Qdrant records without a `qdrant-memory` MCP dependency, but do not override lifecycle provider authority. Neither integration exposes a service SDK, mutates Jira, Taskwarrior, or Plane, or indexes Qdrant without explicit corpus-store authority. Vestige has no lifecycle write, recall, per-node read, or fallback role. Governance records currently remain Git-tracked under `docs/decisions/`; later indexing is optional. `/ima:cycle` independently verifies lifecycle evidence before its explicitly confirmed single-tracker close. See [`docs/foundation/FNR-3016.md`](docs/foundation/FNR-3016.md), [`docs/serena-lifecycle-provider.md`](docs/serena-lifecycle-provider.md), and [`docs/foundation/FNR-3036.md`](docs/foundation/FNR-3036.md) for contracts, ordering, security boundaries, limitations, and live acceptance.
