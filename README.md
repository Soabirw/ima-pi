...
The reviewed independent Serena lifecycle record/client/provider can persist immutable evidence with one write and exact direct read-back, get or read-only reconcile exact references, and recall up to 20 exact verified records. It requires an existing registered canonical Serena project using the default local `.serena` layout with no effective ignored-memory patterns; unsupported protocol, project/layout/visibility failures, conflict or uncertain writes, overflow/incomplete recall, and cancellation fail closed. It does not select a provider, route lifecycle work, change preferences/pins, fall back, repair, retry uncertain writes, or create Serena projects/configuration. T9 separately owns user confirmation, selection, fallback, pins, live routing, and dedicated-session construction. See the [provider contract, recovery, and live-acceptance boundary](docs/serena-lifecycle-provider.md).
...
`BOOKSTACK_BASE_URL` and compatible `BOOKSTACK_ORIGIN` are **non-secret variables**; `BOOKSTACK_TOKEN_ID` and `BOOKSTACK_TOKEN_SECRET` are **secrets**. Provider locators, recovery descriptors, and native continuation-lock paths are **local-only values**. This provider has no **platform binding**.
