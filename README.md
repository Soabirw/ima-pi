...
- `/ima:closeout [completed-lifecycle-source]` is a separately invoked manual terminal lifecycle closeout after verified documentation evidence. It proposes Git, optional project release preparation, tracker closure, and lifecycle persistence as itemized actions, then executes only actions individually confirmed by a human; it never invokes `/ima:cycle` or auto-dispatches from document.
