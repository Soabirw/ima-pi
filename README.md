...
- `/ima:new [high|xhigh|plan]` creates a TUI-only fresh session only from an existing regular-file parent session. Bare `/ima:new` applies no explicit role or phase route; `high` applies `HIGH`; `xhigh` applies only `XHIGH`; `plan` applies the effective `plan` route. Under `--no-session`, or before the parent session is persisted, it fails closed with no replacement or bootstrap. A successful replacement runs resolved Serena then Vestige bootstrap bodies and leaves an unsubmitted `/ima:plan <story-or-task-source>` hint.
