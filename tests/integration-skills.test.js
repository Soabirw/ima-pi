...
test("integration skills retain Pi-native safety and workflow boundaries", async () => {
  ...
  assert.match(vestige, /explicitly cited `vestige:<UUID>`/i);
  assert.match(vestige, /vestige_memory.*action:\s*"get".*id/i);
  assert.match(vestige, /Do not use Vestige for routine preference bootstrap.*preference writes.*lifecycle persistence.*lifecycle recall.*per-node lifecycle reads/is);
  assert.match(vestige, /do not use\s+`session_start` or broad `recall` to load routine preferences/i);
  ...
  assert.match(vestige, /\/ima:memorize.*global `AGENTS\.md`/is);
  assert.match(vestige, /Qdrant manifest plus verified detail\s+chunks/i);
