...
test("shared instruction skills retain their bounded contracts", async () => {
  const [memory, preferences, security, vision, delegation] = await Promise.all(
    skills.map((name) => read(`skills/${name}/SKILL.md`)),
  );
  ...
  mustContain(vision, [
    ...
    /imagePaths/,
    /evidence-only/i,
    /smallest concrete replacement/i,
  ], "vision handoff");
  mustContain(delegation, [
    /ima_delegate/,
    /one to four/i,
    /self-contained/i,
    /writeScope/,
    /fail-closed/i,
    /Retry at most once/i,
  ], "delegation contract");
});
...
test("shared instruction wiring documents dispositions and references the vision skill", async () => {
  ...
  mustContain(wiring, [
    ...
    /js-fp/,
    /No prompt body is changed by this unit/,
  ], "wiring document");
  assert.match(agent, /^skills: \[ima-vision-handoff\]$/m);
  assert.doesNotMatch(agent, /vision_handoff/);
});

test("review and workflow security contracts remain explicit", async () => {
  const [review, lifecycle, delegation] = await Promise.all([
    read("skills/code-review/SKILL.md"),
    read("skills/ima-lifecycle-contract/SKILL.md"),
    read("skills/ima-delegation-contract/SKILL.md"),
  ]);

  mustContain(review, [
    /Security data-flow inventory/i,
    /source to sink/i,
    /If evidence.*fail closed/is,
    /validation.*authorization.*parameterization.*contextual encoding/is,
  ], "code review");
  mustContain(lifecycle, [
    /Unresolved security findings.*resolution and rereview/is,
    /closeout.*Critical or\s+Warning security finding remains unresolved/is,
  ], "lifecycle contract");
  mustContain(delegation, [
    /applicable security.*boundary work/is,
    /validation, authorization, CSRF, sink-encoding, parameterization, and fail-closed/is,
  ], "delegation contract");
});
