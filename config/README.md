...
IMA configuration is schema-versioned JSON with exactly `schemaVersion`, optional `profile`, and optional `models`. Model roles are `HIGH`, `MID`, `LOW`, and `vision`; every mapping has non-empty `provider` and `model`, with optional Pi `thinking` (`off`, `minimal`, `low`, `medium`, `high`, `xhigh`, or `max`). Unknown keys, credentials, endpoints, and arbitrary provider settings are rejected.
