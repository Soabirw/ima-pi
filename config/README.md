...
`/ima:new` is TUI-only, requires a persisted parent session, and accepts exactly no selector, `high`, `xhigh`, or `plan`. It creates a parent-linked replacement session without copying arbitrary old-session state: `high` applies the effective `models.HIGH` role, `xhigh` applies only the effective `models.XHIGH` role, `plan` applies the effective `phases.plan` route, and no selector applies neither. Under `--no-session`, it fails closed and requires restarting Pi without `--no-session`. A successful replacement injects Serena before Vestige and leaves an unsubmitted planning hint; route or bootstrap failure does not auto-plan or downgrade.
