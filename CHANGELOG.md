...
All notable changes to `ima-pi` are documented here. Historical entries are backfilled; release dates and tags are added when a release is cut.
...
- Added a parent-minted, assignment-scoped typed `verifications` capability for one trusted `write`/`test-write` assignment with a declared `test` tool. It admits only snapshotted/revalidated npm or Composer manifest scripts, fixed no-shell argv, preapproved arguments, and parent-selected `1,000`–`360,000` ms (six-minute) timeouts; the child can provide only an ID.
- Added an explicitly operator-confirmed `plane:assign-unassigned` command that assigns only currently unassigned items in one exact project to one exact workspace member after complete bounded discovery and per-item rereads. The operation patches items sequentially, reports changed candidates without overwriting them, rejects malformed identity or assignment evidence before dependent writes, and documents Plane's lack of conditional assignment as a best-effort concurrency limitation.
- Added three-tier lifecycle content screening: blocks definite credentials before effects, safely adjudicates location-only warnings, fails operations after 16 findings, completely screens supported UTF-8 content through 160,000 bytes, and applies defense-in-depth screening to provider reads and writes. Decisions remain process-local and owner/checkout-bound, with no retry, fallback, authority, or pin change.
- Added discoverable `ima-delegated-bash` guidance and mandatory fresh/continued delivery for Bash-capable specialists, preserving conditional sequencing through separately authorized commands while leaving adapter enforcement authoritative.
...
- Added provider-neutral, read-only `ima_lifecycle_recall` and `ima_lifecycle_get` tools: bounded verified descriptors and one complete verified artifact respectively. Reads derive internal authority from the durable pin, or exact all-phase historical Qdrant only while genuinely unpinned; closed references are proof rather than bearer authorization, and failed verification returns no partial result or fallback.
...
- Added report-bound Qdrant lifecycle reset through `/ima:qdrant-lifecycle-reset` and `ima_qdrant_lifecycle_reset`: its strict object-root routes reject mismatched or extra input; mutation-free prepare creates protected checkout-local archive/report evidence from a complete exact-key schema-v1/v2 inventory; execute validates the exact report path/hash, pin, archive, and inventory, requires trusted intent then post-verified-snapshot deletion confirmation, deletes only report-listed UUIDs, and clears recovery only after direct and exhaustive absence proof. Read-only reconciliation is the uncertain-outcome path; ordinary lifecycle operations reject during recovery and share a checkout-local lease. There is no retry, fallback, migration, broad deletion, automatic repin, BookStack/tracker/closeout action, live-provider, or snapshot-restore acceptance claim.
- Added Serena lifecycle-provider groundwork, now consumed by the integrated authority layer: immutable persistence with direct read-back, exact get/read-only reconcile, and bounded recall in an existing registered project. No live Serena/MCP acceptance is claimed.
...
- Added a TUI-only, bounded-confirmation recovery path for a retained BookStack lifecycle writing attempt: stable discovery confirms an exact existing page without a POST, or records a durable one-shot checkpoint before at most one page POST; it never provisions, deletes, abandons, retries, or falls back.
...
- Added sanitized local BookStack v25.12.3 API references: the `/api/docs.json` migration reference and an API-informative Markdown companion packaged with and linked from the main BookStack skill. They support installed-version request/response contracts and URL-reading guidance, require revalidation after BookStack upgrades, and are not a complete or timeless API specification; sensitive browser-complete HTML and identity-bearing assets are intentionally not retained.

### Changed

- Made the default Node test command provider-free and unit-focused by preloading a writable, secret-free fetch-denial guard and restricting it to `tests/*.test.js`; moved the opt-in TTS and Vestige migration acceptance checks to `tests/live/`, preserved their gates, and documented them as separate runs.
- Made provider-free package validation deterministic with `npm run verify:package-dry-run`, and moved BookStack integration/migration fakes to virtual timing without changing production pacing.
- Updated `/ima:gateway-probe` child policy and verification contract: only ordered read-only Serena `serena_activate_project` and `serena_get_current_config` MCP calls are allowed; Vestige calls are rejected; native Qdrant logical-store/direct-get verification covers a dedicated non-production record; and `ima_corpus_status` remains the separate read-only prerequisite check.
- Corrected active memory-routing documentation: Pi's global `AGENTS.md` owns current preferences, Serena owns stable project context, Vestige is limited to cited legacy evidence and the separate T7 migration, and formal lifecycle evidence uses pin-aware lifecycle tools; `/ima:new` documentation now describes Serena bootstrap before phase-mapped skills.
...
- Hardened typed verification timeout/cancellation: it immediately latches unsafe state, waits no more than 1,000 ms for observed cleanup settlement, reports cleanup-unverified instead of false settlement, observes late rejection, and prevents later effects, retry, and reuse.
- Hardened typed verification output sanitization before 200-line/16 KiB bounds in both result channels for supported structured sensitive fields and complete supported authorization-header values; focused continuations never inherit parent-minted typed test authority.
- Fixed Serena MCP session handling: capability discovery and all accepted calls stay on one connection; session-required servers load instructions before other effects, pass private state only to calls whose discovered schema requires it, and accept only agreeing valid initialization/instruction carriers (including the exact bounded Serena instruction marker and valid structured-only SDK normalization); JSON-shaped text is validated before opaque text; unsupported schemas block before effects; legacy callers retain exact activation-first compatibility; context reads all six standard memories including `memory_maintenance`; lifecycle persistence retains immutable one-write/direct-read-back and bounded no-retry/no-fallback behavior; and package-owned child bootstrap hides private session state and checkout. A live read-only context bootstrap now verifies the current configured Serena contract; no live write or production-service acceptance is claimed.
- Fixed manual closeout to partition exact lifecycle evidence by phase, separately recall decision and conditional prior-closeout evidence, treat each phase's exact 50-descriptor result as saturation, and preserve unchanged descriptor-to-get verification.
- SKYNET-247: Raised lifecycle recall requests to `1–50` (default `50`); successful public results remain capped at 49 descriptors, while an internally complete terminal 50-record result is saturated. A valid non-null Qdrant continuation blocks before direct reads with `provider_verification_failed` (`step: recall`, `reason: recall_overflow`), preserving `response_invalid` malformed-continuation and `record_incomplete` chunk failures.
- Hardened pinned direct-anchor lifecycle-context hydration and BookStack recall: resolve hierarchy once, sequentially full-read verify candidates, block embedded-placement conflicts without partial evidence, and provide safe distinct HTTP and recall-overflow diagnostics.
- SKYNET-249: Fixed BookStack lifecycle recall so unmanaged same-chapter pages—including migration pages and empty-slug drafts—no longer block recall when neither name nor slug claims canonical `phase-UUID` identity; any canonical name-or-slug claimant remains fully authoritatively verified and fails closed on invalid evidence.
- Fixed TEST-005 by narrowly allowing comma-delimited lowercase bearer-role prose through a structural dictionary-free rule while populated headers, assignments, standalone token-shaped values, token signatures, private keys, and mixed definite content remain blocked.
- Fixed BookStack migration client request pacing: every client API path now uses one FIFO request-start schedule with an immediate first start, no idle credits, a strict positive `1,100` ms default between later starts, monotonic/injectable timing, issuance-time timeout, and fail-closed cancellation/timing. Cleanup composes operation and client cancellation signals. The operator-selected interval is per-client only—not BookStack/proxy quota evidence or a `429` guarantee; independent clients, processes, runs, and unrelated traffic remain uncoordinated, while unexpected `429` retains the existing systemic breaker with no retry or automatic adjustment.
- Fixed BookStack lifecycle request handling with one process-local FIFO admission lane shared by lifecycle GET, POST, PUT, and permitted GET retries: starts are at least 1,100 ms apart with no idle burst credit, at most 64 requests wait for at most 120 seconds, and operation-local cancellation reaches queued and dispatched initial and pinned persistence without cancelling peers. Timeouts begin at dispatch; hierarchy and placement reads are sequential; failed unconsumed responses are discarded; and lifecycle GETs make at most one retry for `429`/`500`/`502`/`503`/`504`, recognized transient transport failures, or timeout, honoring `Retry-After` only through 10 seconds. Persistent rate limits return explicit safe diagnostics, while POST/PUT writes never replay. A page request rejected before dispatch is no-write only with read-only placement; any earlier provisioning or membership mutation, or any dispatched page write, remains possible-write. The lane is process-local only and intentionally adds latency; it neither coordinates distributed callers nor guarantees rate-limit avoidance.
- Fixed document readiness across the shared lifecycle contract and `/ima:document` and `/ima:soft-cycle`: before `READY`, reconcile complete verified lifecycle-delivered changes with `CHANGELOG.md`; detect omissions, inaccuracies, duplicate or unsupported claims; preserve unrelated and released history; change only authorized lifecycle prose when `CHANGELOG.md` is an exact approved target; accept no change only with evidence; reread the changelog and inspect the scoped diff after reconciliation or correction; otherwise fail closed.
- Hardened delegation write safety: authorized scoped edit/write operations are serialized per assignment; project-contained absolute paths are safely normalized, and canonical resolution safely retries a missing-to-existing race without weakening containment, documenter lexical/canonical documentation authority, or symlink protections; trusted, fully correlated pre-effect denials may recover, while incomplete, cancelled, uncertain, or post-effect evidence fails closed with bounded actionable causes. Continuation `writeScope` remains fingerprint-bound and immutable before execution; exceptional exits block reuse or retry until evidence settles, while safe parent-directory creation remains supported.
- Fixed lifecycle reads so a descriptor from `ima_lifecycle_recall` can be passed unchanged to `ima_lifecycle_get`, including its bounded `summary`; failures preserve existing error codes/messages and may add bounded diagnostics: allowlisted provider categories with optional safe phase/reason metadata, four fixed get-side `verification` categories, or `unexpected_failure` at runtime. Diagnostics reveal no provider identity, location, credentials, resource identifiers, artifact content, raw upstream errors, or stack traces.
- Corrected SKYNET-245 P/R/S lineage authority: P remains the verified provider-pin anchor, R is the original lifecycle-seed root, and S is the stable source identity. Only rootless `plan` and `decision` seeds are accepted before provider effects; both establish R, decision-first continuations preserve exact R/S, and a distinct approved technical `plan` remains required for cycle adoption and closeout. Every other rootless phase remains fail-closed with no repair, migration, repinning, fallback, or tracker effect.
...
- Hardened BookStack page discovery to tolerate unrelated valid empty-slug drafts while treating target drafts as conflicts; authoritative reads and creates remain strict.
- Hardened BookStack lifecycle pagination: each transient list scan requests `sort: +id`, accepts a nondecreasing latest observed total only under higher-ID append growth, requires complete pages of up to 500 entries and positive strictly increasing IDs while preserving valid gaps, and rejects malformed, incomplete, shrinking, duplicate, overlapping, reordered, or bounds-invalid enumeration without accepting partial authority. Responses remain capped at 4 MiB and scans at 10,000 entries; offset pagination is not snapshot-isolated and cannot universally detect delete-and-append races. Failed page discovery results in zero lifecycle-page POSTs only; earlier placement effects may exist, and recovery confirmation still requires matching discoveries.
- Made BookStack lifecycle page verification tolerate only the provider's omission of one terminal LF while reconstructing canonical evidence and retaining actual-byte locator hashes; all broader content or whitespace drift still blocks.
- Fixed pinned BookStack lifecycle recall to verify the full locator before passing its exact placement projection; malformed or mismatched evidence blocks without fallback.
- Removed redundant provider-selection and placement-confirmation prompts from pinned BookStack persistence: continuations directly verify and reuse the exact placement without provisioning or membership mutation; first organization-visible placement still requires explicit consent, and pin mismatch or provider failures fail closed without retry, fallback, migration, switching, or mixing.
...
- Reused the established `BOOKSTACK_BASE_URL`, replaced repeated catalog scans with one bounded catalog, and stopped further writes on systemic apply failures without assuming ownership of administrator-managed guest/public policy.
