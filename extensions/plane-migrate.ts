...
const discoverDestinations = async ({ client, workspace }) => {
  ...
  return Promise.all(projects.map(async (project) => {
    const [states, items] = await Promise.all([
      client.listProjectStates({ workspace, projectId: project.id }),
      client.listProjectItemsByExternalSource({
        workspace,
...
const messageFor = (error) => {
  ...
  return "Migration preparation could not be completed safely.";
};
...
export default function planeMigrate(pi: ExtensionAPI) {
  pi.registerCommand("ima:plane-migrate", {
    description: "Interactively prepare a zero-write Taskwarrior-to-Plane migration.",
    handler: async (args, ctx) => {
      if (args.trim() !== "") {
        notify(ctx, "Usage: /ima:plane-migrate");
        return;
      }
