...
Writes including smart_ingest are explicit mutations requiring authority. `ima_lifecycle` persists and semantically verifies lifecycle artifacts directly through Vestige MCP: it calls `smart_ingest`, then requires a successful receipt and `recall` nonce/identity/outcome match. Callers must continue to use `ima_lifecycle` rather than bypassing its lifecycle receipt protocol.
