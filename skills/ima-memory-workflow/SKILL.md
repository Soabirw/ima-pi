...
3. For lifecycle evidence, load `ima-lifecycle-contract` and use its pin-aware route, never Vestige:
   ...
   - when pinned, accept only that provider's direct verified recall/get/reconcile evidence and preserve its `artifactId`, `recordKey`, and provider-native reference. Both guided and autonomous phases use the exact provider-native pinned placement without provider selection or placement confirmation; never query Qdrant as fallback or mix providers;
   - when unpinned, exact bounded Tier-1 Qdrant recall across all phases decides historical authority. A verified hit requires Qdrant; a verified empty result proceeds only under the lifecycle contract's recommendation. Autonomous storage flow pauses for explicit user consent only when the first unpinned, organization-visible BookStack placement lacks it. Consent does not pin; only verified persistence plus provider-native direct read-back does;
   - Provider unavailability, mismatch, unknown writes, or persistence failure is `BLOCKED`; do not retry a `BLOCKED` write, fall back, migrate, switch providers, or mix history;
...
BookStack consent, provider and placement rules, historical-Qdrant authority, durable pins, and
fail-closed evidence order; `ima_lifecycle` owns formal persistence and direct verification through
that authority. Neither this workflow nor a phase prompt selects or approves a provider, or repeats a
storage approval after a verified pin. Pi's global `AGENTS.md` owns routine current preferences;
Tier-1 Qdrant remains institutional knowledge rather than an unconditional lifecycle backend;
Vestige remains a bounded cited-legacy and migration source, never a routine preference bootstrap or
lifecycle fallback. Use this skill to select the right memory system and evidence order; do not
duplicate tool-owned setup or persistence procedures.
