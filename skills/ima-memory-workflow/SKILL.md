...
3. For lifecycle evidence, load `ima-lifecycle-contract` and use its public pin-aware read pair, never Vestige:
   ...
   - treat every returned descriptor as a selector only. For each required descriptor, call `ima_lifecycle_get` with only its exact `lifecycleKey`, `phase`, and `artifactId`; never reconstruct or send the descriptor's proof fields. The package freshly recalls that exact phase, requires one unambiguous non-saturated match, projects the closed proof internally, and then performs the exact get. Accept only the complete result after verifying its lifecycle key, phase, source identity and completion marker, returned `artifactId`, logical `recordKey`, content hash, read reference, and the phase-specific prerequisite;
   ...
   - for pinned evidence, verify `P` as the provider-pin anchor, derive lifecycle-seed root `R` and stable source identity `S` only from its complete record, then accept only records with exact R/S. Only `plan` and `decision` may be rootless seeds; a rootless phase other than `plan` or `decision` P blocks unchanged. A decision never satisfies approved technical-plan selection: cycle adoption and closeout require a distinct approved `plan`. Never infer R from ordering, a receipt, tracker data, or `priorArtifactIds`.
