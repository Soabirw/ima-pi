...
1. Run a dry-run against the versioned spec. It performs read-only Qdrant and working-tree Markdown enumeration, writes an immutable local report under `.ima/bookstack-migrate/`, and does not call BookStack.
...
Never import the derived `ima-knowledge` chunks. Eligible current Markdown files under `IMA_RAG_ROOT` are the source regardless of Git status; immutable inventory hashes and repeat enumeration reject source drift. Never delete Qdrant or BookStack source material through this command.
