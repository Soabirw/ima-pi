...
`recall({ lifecycleKey, phase?, limit? }, signal?)` is also read-only. It scopes only to `sha256(lifecycleKey)`, accepts an optional valid phase, defaults `limit` to 50, and accepts only limits 1–50. It enumerates at most 10,000 directory entries, verifies every lifecycle-looking artifact/receipt pair, rejects malformed names, unmatched pairs, duplicate artifact IDs, invalid records, scope changes, or lease activity, and returns no partial authoritative result. A missing scope returns `[]`; a successful adapter result is sorted by receipt filename, optionally phase-filtered, and then limited. An internally complete adapter result may contain exactly 50 records, but the provider-neutral public recall treats that count as saturated and never returns more than 49 descriptors.
