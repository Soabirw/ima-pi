...
This lineage policy does not change the public provider-neutral read signatures below. Its approved evidence is canonical source `plane:ima:SKYNET-230`, final approved rereview `d2ffc65d-35ae-57cd-bfc2-ed9f3c65c69c`, and assessment `direct:d7f62171-e139-422b-a05b-7b59b929fef1`.
...
`ima_lifecycle_get({ lifecycleKey, phase, artifactId, recordKey, contentHash, reference, summary? })` reads one complete verified artifact. A descriptor returned by recall can be passed unchanged to `get`, including its `summary`; prior recall or a cache is not required. `document` is an exact public phase and never expands to `closeout`.
