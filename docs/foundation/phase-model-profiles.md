...
Successful cycle-owned specialist session records are written to ignored local `.ima-cycle/agent-sessions.json`; follow-up-eligible specialists created by direct/manual phases are written to `.ima-cycle/direct-agent-sessions.json`. Cycle references remain owner-bound, while direct/manual references can be restored across sessions. Rereview uses `ima_agent_follow_up` to continue an eligible original reviewer and never creates a new reviewer. Registry writes use bounded exclusive local sidecars; a continuation leases its session file before opening it and returns `session_follow_up_busy` if already leased. It holds that lease through settlement, durable update, and disposal; verified cleanup is required before release, while `session_cleanup_unverified` retains the lease and blocks retry. It never steals a stale lock, and operator cleanup requires confirmation of no live owner. `commands.cycle` and other profile mappings are **non-secret configuration**; provider credentials are **secrets** and are not stored in profile or local session records. Registry paths and lock sidecars are **local-only values**; this behavior has no **platform binding**.
...
`/ima:new` accepts bare invocation, role selectors, and discovered IMA command names. A command selector uses the same lookup and may start unchanged when it has no route; an explicitly resolved route remains fail-closed for busy, unavailable, unauthenticated, unsupported, or rollback-failed switching. It preserves the existing parent-link, Serena -> mapped-skill bootstrap ordering, and Pi-native global `AGENTS.md` context.
