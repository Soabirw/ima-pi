...
TUI-only `/ima:new` accepts bare invocation, the role selectors `low`, `mid`, `high`, and `xhigh`, and every canonical phase selector. Bare invocation applies no explicit route; role selectors apply the matching configured role and phase selectors apply the matching canonical phase route. A verified regular-file parent is forwarded as the native parent link; a missing, unwritten, inaccessible, or non-file path is omitted so the replacement remains usable without a parent link. The fresh session records only the selector and a sanitized route result, rejects selector/route mismatches, applies any selected route before ordered Serena and Vestige bootstrap messages. Any route, cancellation, or bootstrap failure blocks without fallback or automatic planning.
